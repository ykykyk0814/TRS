...
from app.dto.preference import PreferenceRequestDTO, PreferenceUpdateDTO, PreferenceResponseDTO
...
async def create_preference(
    preference_data: PreferenceRequestDTO, 
    user: User = Depends(current_user)
):
    ...
    preference = await preference_service.create_preference(preference_data.model_dump())
    return PreferenceResponseDTO.model_validate(preference)
...
async def delete_preference(
    ...
    if existing_preference.user_id != user.id and not user.is_superuser:
        raise HTTPException(status_code=403, detail="Can only delete your own preferences")
